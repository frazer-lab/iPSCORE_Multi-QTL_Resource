...
calculate_peer_factors = function(config, expdata)
{
    ...
    #geno_pcs = add_rownames(fread("/projects/CARDIPS/analysis/epigenome_resource/eqtls/prepare_wgs/pca/pca.eigenvec", data.table = F) %>% select(-V1))
    #colnames(geno_pcs) = paste0("PC", c(1:ncol(geno_pcs)))
    
    #print(geno_pcs[1:5, 1:5])
    
    #message(paste(Sys.time(), "calculating peer factors using", nrow(normalized), "elements"))
   
    model = PEER()
    PEER_setPhenoMean (model, t(as.matrix(normalized))  )
    PEER_setNk        (model, config$peer_factors_n_peer)
    
    #message(paste(Sys.time(), "setting covariates"))
    #PEER_setCovariates(model, as.matrix(geno_pcs)       )
    PEER_update       (model                            )
...
#run_qtl = function(config, element_id, tmp_folder, phenotype_info, expdata, gtinfo, gtdata, h5_file, cov_file, kin_file, qtl_file_tmp, geneloc_file, snploc_file, gt_file)
#{   
#    suppressWarnings(write_h5_file(element_id, phenotype_info, expdata, gtinfo, gtdata, h5_file))
    
#    command_py = paste(config$python, paste(config$out_folder, "notebooks", "run_limix.py", sep = "/"), tmp_folder, element_id, "scan", "normal")
...
#    n_independent     = find_redundancy(gtdata, 0.8)
#    fdrdata$fdr       = min(c(1, fdrdata$pval * n_independent))
#    fdrdata$tests     = n_independent
...
# ------------------- Plotting functions
plot_box_qtl = function(element_id, snp)
{
